...
int main() {
	...
	for (int i = BASE_INDEX; i < BASE_INDEX + n_subjects; i++) {
		subject_names[i] = new char[MAX_SUBJECT_NAME_LENGTH + 1] - BASE_INDEX; // +1 для \0
	}
#else
	...
	for (int i = BASE_INDEX; i < BASE_INDEX + n_subjects; i++) {
		subject_names[i] = (char *)calloc(MAX_SUBJECT_NAME_LENGTH + 1, sizeof(char)) - BASE_INDEX; // +1 для \0
	}	
#endif
